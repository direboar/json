...
public class Atom2Json {
	...
	private static final String[] NON_ARRAY_ATTRIBUTE_NAMES = new String[] {
			"email", "generator", "icon", "id", "logo", "name", "published",
			"rights", "title", "subtitle", "summary", "updated", "uri" };
	 ...
	 *            　処理対象のJsonObject
	 * @param parentElement
	 *            parentJsonに対応するatomのElementオブジェクト
	 */
		...
		for (int j = 0; j < nodeChildren.getLength(); j++) {
			...
			if (childNode instanceof Element) {
				...
				if (isArray(childElement.getTagName())) {
					JsonArray jsonArray = (JsonArray) parentJson
							.get(elementName);
					if (jsonArray == null) {
				...
				// TODO
				// 実際はJsonObjectに変換されるケースとしては属性を持つが子要素がないケースも含まれるので、今の処理は処理効率上ちょっとイマイチ
	...
	/**
	 * XMLElementをJsonElementに変換する。 型の決定ルール:
	 * <ol>
	 * <li>Elementに属性がある場合はJsonObjectに変換する
	 * <li>
	 * <li>Elementの子ノードにElementがある場合は、JsonObject似変換する
	 * <li>
	 * <li>上記以外の場合は、Element内のテキストコンテンツを扱う文字列型として扱う
	 * <li>
	 * </ol>
	 * JsonObjectの場合の属性値決定ルール
	 * <ol>
	 * <li>Elementに属性がある場合は、全てJsonObjectの属性として設定する。
	 * <li>
	 * <li>Elementの子ノードにElementがない場合は、
	 * Element内のテキストコンテンツをcontentという名前の属性でJsonObjectに設定する
	 * <li>
	 * </ol>
	 * 
	 * @param element
